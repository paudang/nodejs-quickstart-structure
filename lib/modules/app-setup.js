...
export const processAllTests = async (targetDir, config) => {
    ...
    const processDir = async (currentDir) => {
        ...
        for (const item of items) {
            ...
            } else if (itemPath.endsWith('.spec.ts.ejs') || itemPath.endsWith('.spec.js.ejs')) {
                const relativePath = path.relative(srcDir, itemPath);
                const targetTestPath = path.join(testsDir, relativePath.slice(0, -4));
                
                await fs.ensureDir(path.dirname(targetTestPath));
                const content = ejs.render(await fs.readFile(itemPath, 'utf-8'), config);

...
export const renderDynamicComponents = async (templatePath, targetDir, config) => {
    ...
    // Advanced Logger Modularization (ELK / Enterprise)
    const loggerExt = language === 'TypeScript' ? 'ts' : 'js';
    const loggerTemplatePath = path.join(templatePath, '../../common/src/utils', `logger.${loggerExt}.ejs`);

    if (await fs.pathExists(loggerTemplatePath)) {
        let utilsDestDir = path.join(targetDir, 'src', 'utils');
        if (architecture === 'Clean Architecture') {
            utilsDestDir = path.join(targetDir, 'src', 'infrastructure', 'log');
        }
        await fs.ensureDir(utilsDestDir);

        const loggerContent = ejs.render(await fs.readFile(loggerTemplatePath, 'utf-8'), { ...config });
        await fs.writeFile(path.join(utilsDestDir, `logger.${loggerExt}`), loggerContent);

        // Render logger spec template
        const loggerSpecTemplatePath = path.join(templatePath, '../../common/src/utils', `logger.spec.${loggerExt}.ejs`);
        if (await fs.pathExists(loggerSpecTemplatePath)) {
            let testUtilsDestDir = path.join(targetDir, 'tests', 'unit', 'utils');
            if (architecture === 'Clean Architecture') {
                testUtilsDestDir = path.join(targetDir, 'tests', 'unit', 'infrastructure', 'log');
            }
            await fs.ensureDir(testUtilsDestDir);
            const specContent = ejs.render(await fs.readFile(loggerSpecTemplatePath, 'utf-8'), config);
            await fs.writeFile(path.join(testUtilsDestDir, `logger.spec.${loggerExt}`), specContent);
        }
    }

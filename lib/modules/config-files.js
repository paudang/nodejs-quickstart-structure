...
export const renderDockerCompose = async (templatesDir, targetDir, config) => {
    ...
    if (config.withELK) {
        const elkComposePath = path.join(targetDir, 'docker-compose.elk.yml');
        const elkTemplate = await fs.readFile(path.join(templatesDir, 'common', 'docker-compose.elk.yml.ejs'), 'utf-8');
        const elkContent = ejs.render(elkTemplate, { ...config });
        await fs.writeFile(elkComposePath, elkContent);
    }
};

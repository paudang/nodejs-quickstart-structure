...
export const setupTerraform = async (templatesDir, targetDir, config) => {
    ...
    const cloudProviderPath = config.cloudProvider ? config.cloudProvider.toLowerCase() : 'aws';
    const terraformSourceDir = path.join(templatesDir, 'common/terraform', cloudProviderPath);
    ...
    try {
        ...
        if (config.database === 'None') {
            ...
            if (await fs.pathExists(mainTfPath)) {
                ...
                content = content.replace(/# --- Data Layer.*?---[\s\S]*?module "database" \{[\s\S]*?\}\n/g, '');
        ...
        if (config.caching !== 'Redis') {
            ...
            if (await fs.pathExists(mainTfPath)) {
                ...
                content = content.replace(/# --- Cache Layer.*?---[\s\S]*?module "cache" \{[\s\S]*?\}\n/g, '');
                await fs.writeFile(mainTfPath, content);
            }

...
export const getProjectDetails = async (options = {}) => {
    const questions = [
        ...
        },
        {
            ...
            message: 'Enable Advanced Options (Authentication, Terraform, Resilience, etc.)?',
            choices: ['No', 'Yes'],
            default: 'No',
        ...
        },
        {
            ...
            choices: [
                { name: 'Timeout (Prevent resource hanging)', value: 'Timeout' },
                { name: 'Advanced Retry (Exponential Backoff)', value: 'Retry' },
                { name: 'Circuit Breaker (Isolate failing downstream services)', value: 'CircuitBreaker' }
            ],
            default: [],
            when: (answers) => {
                const advanced = options.advancedOptions !== undefined ? options.advancedOptions : answers.advancedOptions;

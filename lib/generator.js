...
export const generateProject = async (config) => {
    ...
    console.log(`
      ...
      5. docker-compose up -d${config.database !== 'None' ? ' db' : ''}${config.caching === 'Redis' ? ' redis' : ''}${config.communication === 'Kafka' ? ' kafka' : ''} (To start DB/Infrastructure)${config.withELK ? '\n      5b. docker-compose -f docker-compose.elk.yml up -d (To start ELK Stack)' : ''}

...
In the root of your generated project, run the specific Docker Compose file for ELK:```bash
...
You can check if Elasticsearch is running by visiting:`http://localhost:9200`You should see a JSON response with the cluster name and version`8.14.0`.

### 3. Start Your Node.js Application

Start your Node.js application normally:::: code-group
```bash [npm]
npm run dev```
```bash [pnpm]
pnpm dev```
...
> [!TIP] Tìm hiểu sâu về ELK Stack
> Bạn muốn hiểu rõ kiến trúc đằng sau cách triển khai này? Hãy đọc bài viết phân tích sâu về hạ tầng của chúng tôi đăng trên **System Weakness (Medium)**:
> 

...
6. **Database**: Chọn **MySQL**, **PostgreSQL**, **MongoDB**, hoặc **None**.
7. **Database Name**: Chỉ định tên cơ sở dữ liệu của bạn (nếu có chọn cơ sở dữ liệu).
8. **Caching**: Thêm **Redis** hoặc **Memory Cache**.
9. **Authentication**: Hỗ trợ gắn thêm **JWT** và **(OAuth2 - Google/GitHub)**.
...
14. **Background Jobs**: Tùy chọn hàng đợi công việc **BullMQ** + **Bull-Board** (Yêu cầu Redis).
